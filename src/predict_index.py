...
def predict_per_symbol(model, feature_params: FeatureParams, label_params: LabelParams, feature_cols: List[str], symbols: List[str], interval: str) -> List[Dict[str, Any]]:
    ...
    for sym in symbols:
        ...
        if len(ds) == 0:
            continue
        # Split feature columns into base and symbol one-hots, then build row accordingly
        ...
        out: Dict[str, Any] = {"symbol": sym, "action": action, "confidence": float(proba[pred_idx])}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Predict per-symbol actions and compute index WSS")
    ...
    parser.add_argument("--signal_buy", type=float, default=0.5, help="WSS >= threshold -> BUY")
    parser.add_argument("--signal_sell", type=float, default=-0.5, help="WSS <= threshold -> SELL")
    ...
    rows = predict_per_symbol(model, fparams, lparams, feature_cols, symbols, args.interval)
    ...
    with mlflow.start_run(run_name="index_signal"):
        ...
        wss_json.write_text(json.dumps({"wss": wss, "signal": signal, "detail": wss_info}, indent=2), encoding="utf-8")
